...
+ (void)registerWithRegistrar:(NSObject <FlutterPluginRegistrar> *)registrar {
    ...
    instance->obj = [NSClassFromString(@"YYWJIJKPlayPlugin") new];
    [registrar addMethodCallDelegate:instance channel:channel];
    // __sharedInstance = instance;
}

- (void)invokeMethod:(NSObject*)obj methodName:(NSString*)methodName param:(id)param{
    SEL selector = NSSelectorFromString(methodName);
    if ([[obj class] instancesRespondToSelector:selector]) {
        IMP imp = [obj methodForSelector:selector];
        void(*func)(id, SEL, NSDictionary*) = (void *)imp;
        func(obj, selector,param);
    }
}
